...
int SD_init()
{
    ...
    send_and_receive("Sending CMD16", 
    print("Card ready");
   return 1;
}
...
void send_and_receive(char *descr, unsigned char cmd, unsigned long arg,unsigned char crc, unsigned char expected) {
    while(1) {
        send_command(descr, cmd, arg>>24, arg>>16, arg>>8, arg, crc);
