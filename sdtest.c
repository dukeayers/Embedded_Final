...
#define SELECT()	LATBbits.LATB6 = 0	/* MMC CS = L */
#define	DESELECT()	LATBbits.LATB6 = 1	/* MMC CS = H */

#define SOCKPORT	PORTB			/* Socket contact port */
#define SOCKWP		0x800		/* Write protect switch (bit2) */
#define SOCKINS		0x400		/* Card detect switch (bit3) */
...
static volatile
UINT Timer1, Timer2;	/* 250Hz decrement timer */
...
//#define xmit_spi(dat) 	SPI1BUF=(WORD)(dat); do; while(SPI1STATbits.SPITBF)
//static
...
BYTE wait_ready (void)
{
	...
	Timer2 = 125;	/* Wait for ready in timeout of 500ms */
	rcvr_spi();
	do
		res = rcvr_spi();
...
)
{
	...
	//CKP=1,CKE=0,SMP=0
	SPI1STAT = 0;
	SPI1CON1 = 0x0061;		//1:2MHz/16=125KHz 3:2MHz
	SPI1CON2 = 0;
	SPI1STAT = 0xa000;
	...
	if (send_cmd(CMD0, 0) == 1) {			/* Enter Idle state */
		Timer1 = 250;						/* Initialization timeout of 1000 msec */
	...
	if (ty) {			/* Initialization succeded */
		...
		SPI1STAT = 0;
		SPI1CON1 = 0x0063;		//1:2MHz/16=125KHz 3:2MHz
		SPI1CON2 = 0;
		SPI1STAT = 0xa000;
...
//	static BYTE pv;
	...
	n = Timer1;						/* 250Hz decrement timer */
	if (n) Timer1 = --n;
	n = Timer2;
	if (n) Timer2 = --n;
